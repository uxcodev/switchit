...
export default {
  ...
  async mounted() {
    ...
    if (this.leads?.length === 1) {
      ...
      this.lead = all_leads.find(lead => lead.id === this.leads[0])
    ...
    console.log('this.offer_obj', this.offer_obj)
    this.offer_obj.offer_details.start_date = new Date()
    this.offer_obj.offer_details.name = 'Test offer ' + new Date().toISOString().split('T')[0]
    this.offer_obj.offer_details.expiry_date = new Date(new Date().setDate(new Date().getDate() + 30))
