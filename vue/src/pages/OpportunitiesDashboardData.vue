...
export default {
  ...
  setup() {
    ...
    async function loadLeads() {
      ...
      leads.value = leads.value.length ? leads.value : await fake_data.getLeads()
      leadCount.value = leads.value.length;

      console.log("leads: ", leads.value)
      console.log("leadCount: ", leadCount.value)




      console.log('remove these comments to load real data')
      /* 
      let response = await api.getLeads({
        limit: pg.limit,
        skip: 0,
