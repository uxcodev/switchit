...
export default {
  ...
  setup() {
    ...
    // **** TEMPORARILY USE 'CREATED' INSTEAD OF 'ID' ****
    function toggleSelectVisible() {
      ...
      if (selectVisible.value) {
        // Add new leads to selectedLeads array as plain objects
        const newLeads = leads.value.filter((lead) =>
          !selectedLeads.value.some(
            (selectedLead) => selectedLead.id === lead.id && selectedLead.created === lead.created
          )
        );
        selectedLeads.value.push(
          ...newLeads.map((lead) => JSON.parse(JSON.stringify({ id: lead.id, created: lead.created })))
        );
      } else {
        // Remove unselected leads from selectedLeads array
        selectedLeads.value = selectedLeads.value.filter(({ id, created }) =>
          leads.value.find((lead) => lead.id === id && lead.created === created && lead.selected)
        );
      }
    }
    ...
    function updateSelectedLeads(lead) {
      if (lead.selected) {
        selectedLeads.value.push(JSON.parse(JSON.stringify({ id: lead.id, created: lead.created })));
      } else {
    ...
    watch(filtersChanged, () => {
      ...
      filters.value = store.getters.filters;
      loadLeads()
    });
    ...
    function openLeads() {
      ...
      // *** TEMPORARILY USE 'CREATED' INSTEAD OF 'ID' ***
      // extract ids from all selected leads and save them as an array
      let leadIds = selectedLeads.value.map((lead) => lead.id);
      $storeSessionValue('offer_selectedLeads', leadIds, 60);
      store.dispatch("setSelectedLeads", leads.value.filter((lead) => lead.selected));
    ...
    watch(() => pg.currentPage, async () => {
      ...
      console.log('leads returned from new page', leads.value)
      console.log('selectedLeads', selectedLeads.value)

      leads.value = leads.value.map((lead) => ({
        ...lead,
        // selected: selectedLeads.value.includes(lead.id), // TEMPORARILY USE 'CREATED' INSTEAD OF 'ID'
        selected: selectedLeads.value.some(
          (selectedLead) => selectedLead.created === lead.created
        ),
      }));
